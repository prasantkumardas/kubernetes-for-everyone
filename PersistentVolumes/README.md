...
#
<b>1) Create persistent volume yaml file :</b>
```bash
...
#
<b>3) Create one more yaml file for persistent volume claim :</b>
```bash
apiVersion: v1
...
#
<b>4) Apply persistent volume claim :</b>
```bash
...
#
<b>5) Create a simple nginx pod yaml to attach volume :</b>
```bash
apiVersion: v1
kind: Pod
metadata:
...
#
<b>7) Now exec into the above the created pod i.e. nginx :</b>
```bash
...
#
<b>8) Go to the path /usr/share/nginx/html and do <mark>ls -lrt</mark> :</b>
```bash
...
#
<b>9) Now let's create a file under /usr/share/nginx/html path :</b>
```bash
...
#
<b>10) Now exit from the pod and ssh into the minikube host :</b>
```bash
exit
```
```bash
...
#
<b>11) Now go the path which you mentioned in PersistentVolume.yaml. In our case it is  <mark>/home/ubuntu/data</mark> and check if the file is present or not :</b>
```bash
cd /home/ubuntu/data
ls -lrt
```
...
#
<b>12) Now let's create one more file under /home/ubuntu/data inside minikube host :</b>
```bash
...
#
<b>13) At last, go to nginx pod and check if minikube-host-pod.txt file is present or not :</b>
```bash
kubectl exec -it <pod> -- sh
```
```bash
